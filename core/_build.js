...
process.on("SIGINT", () => global.shutdown());
process.on("SIGTERM", () => global.shutdown());
process.on("SIGHUP", () => global.shutdown());
...
function refreshState() {
    global.refreshState = setInterval(() => {
        ...
        const appStatePath = resolvePath(process.env.TEMP || '', global.config.APPSTATE_PATH);
                ...
                db.get("APPSTATE_SECRET_KEY")
                    .then((value) => {
                        if (value !== null) {
                            ...
                            const encryptedAppState = aes.encrypt(JSON.stringify(newAppState), APPSTATE_SECRET_KEY);
                            writeFileSync(appStatePath, JSON.stringify(encryptedAppState), "utf8");
                        }
                    })
                    .catch(console.error);
            } else {
                writeFileSync(appStatePath, JSON.stringify(newAppState, null, 2), "utf8");
            }
        } else {
            writeFileSync(appStatePath, JSON.stringify(newAppState, null, 2), "utf8");
        }
...
async function loginState() {
    const appState = await checkAppstate(global.config.APPSTATE_PATH, global.config.APPSTATE_PROTECTION);
